...
async function renderEnvVars() {
    ...
    // Fetch registry to see what keys are required by installed templates
    let requiredKeys = new Set();
    try {
        const res = await fetch('/api/registry');
        const reg = await res.json();
        reg.filter(a => a.installed).forEach(a => {
            if (a.execution && a.execution.env_vars_required) {
                a.execution.env_vars_required.forEach(k => requiredKeys.add(k));
            }
        });
    } catch (e) {
        console.error('Failed to load registry for env vars');
    }
    ...
    if (requiredKeys.size === 0) {
        html = '<p style="color:var(--text-secondary);font-size:0.85rem;margin-bottom:0.5rem;">No active templates require explicit API keys.</p>';
    }

    requiredKeys.forEach(key => {
        const val = savedEnvVars[key] || '';
        html += `
            <div class="form-group">
                <label>${key}</label>
                <input type="password" class="env-var-input" data-key="${key}" value="${escapeHtml(val)}" placeholder="Enter ${key}">
            </div>
        `;
    });

    // Add a custom key/val row for extensibility
    html += `
        <div style="margin-top: 1rem; padding-top: 0.5rem; border-top: 1px dashed var(--border);">
            <label style="font-size:0.8rem;color:var(--text-secondary)">Add Custom Variable</label>
            <div style="display:flex; gap:0.5rem; margin-top:0.25rem;">
                <input type="text" id="customEnvKey" placeholder="KEY_NAME" style="flex:1">
                <input type="password" id="customEnvVal" placeholder="Value" style="flex:2">
                <button class="secondary" onclick="addCustomEnvVar()" style="padding:0.4rem 0.75rem;">Add</button>
            </div>
        </div>
    `;
...
function addCustomEnvVar() {
    const keyInput = document.getElementById('customEnvKey');
    const valInput = document.getElementById('customEnvVal');
    const key = keyInput.value.trim();
    const val = valInput.value.trim();
    if (!key) return;
    if (!config.env_vars) config.env_vars = {};
    config.env_vars[key] = val;
    renderEnvVars(); // Re-render to show it
}
...
async function saveSettings() {
    ...
    document.querySelectorAll('.env-var-input').forEach(input => {
        ...
        if (key && input.value) {
            ...
            delete envVars[key]; // Allow clearing
        }
    });

    try {
        const resConfig = await fetch('/api/config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(config) });
        const resEnv = await fetch('/api/config/env', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(envVars) });

        if (resConfig.ok && resEnv.ok) {
